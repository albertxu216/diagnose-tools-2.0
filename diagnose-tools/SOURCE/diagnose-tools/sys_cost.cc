...
static void do_activate(const char *arg)
{
	...
	memset(&settings, 0, sizeof(struct diag_sys_cost_settings));//将settings字段的内容全部填充为0；
	
	/*将传入的参数通过parse解析并传入setting中；*/
	...
	/*调用系统调用去设置settings*/
	if (run_in_host) {
		ret = diag_call_ioctl(DIAG_IOCTL_SYS_COST_SET, (long)&settings);//跳到sys_cost.c的diag_ioctl_sys_cost函数去实现
	} else {
		ret = -ENOSYS;
		syscall(DIAG_SYS_COST_SET, &ret, &settings, sizeof(struct diag_sys_cost_settings));//跳到sys_cost.c的sys_cost_syscall函数去实现
	}
	...
	ret = diag_activate("sys-cost");//调用misc.cc中的diag_activate函数，通过标准输入流，会调用向/proc/ali-linux/diagnose/controller写入activate sys-cost ，写成功会返回1，将1赋给ret
	if (ret == 1) {
...
static int sys_cost_extract(void *buf, unsigned int len, void *)
{
	...
	case et_sys_cost_detail:
		...
		if (detail->tgid_rcd_scid.tgid<=0 && detail->tgid_rcd_scid.nr_pid==1) {//指定了单进程
			struct record_syscall_id *syscall_id = &detail->tgid_rcd_scid.syscall_id[0];
			printf("指定目标pid: %d\nsyscall_id:",syscall_id->pid);
			for(int j=syscall_id->front;j<syscall_id->count;j++){
				if((j - syscall_id->front)%10 == 0)
					printf("\n");
				printf("%d ",syscall_id->record_scid[j%MAX_SYSCALL_COUNT]);
			}
			printf("\n");
		}
		if(detail->tgid_rcd_scid.tgid!=0 && detail->tgid_rcd_scid.nr_pid!=0){
			printf("指定目标tgid: %d\n",detail->tgid_rcd_scid.tgid);
			struct record_syscall_id *syscall_id = &detail->tgid_rcd_scid.syscall_id[0];
			for(int k=0;k<detail->tgid_rcd_scid.nr_pid && k<MAX_PID;k++){
				printf("\t指定目标pid: %d\n\tsyscall_id:",syscall_id[k].pid);
				for(int j=syscall_id[k].front;j<syscall_id[k].count;j++){
					if((j - syscall_id[k].front)%30 == 0)
						printf("\n\t\t");
					printf("%d ",syscall_id[k].record_scid[j%MAX_SYSCALL_COUNT]);
				}
				printf("\n");
			}
			printf("\n");
