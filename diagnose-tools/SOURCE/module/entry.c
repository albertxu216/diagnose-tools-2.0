		...
		loff_t *offs)
{
	...
	if (strcmp(cmd, "activate") == 0) {
		...
		if (strcmp(func, "run-trace") == 0) {
			...
			activate_pmu();
		}else if (strcmp(func, "tcpstates") == 0) {
			activate_tcp_states();
		}else if (strcmp(func, "udp") == 0) {
			activate_udp();
		} else if (strcmp(func, "resource-image") == 0) {
			activate_resource_image();
		} else if (strcmp(func, "migrate-image") == 0) {
			activate_migrate_image();
		} else if (strcmp(func, "sched-image") == 0) {
			activate_sched_image();
		} else if (strcmp(func, "test-map") == 0) {
			activate_test_map();
		} else if (strcmp(func, "keytime-image") == 0) {
			activate_keytime_image();
		} 
		...
		if (strcmp(func, "run-trace") == 0) {
			...
			deactivate_resource_image();
		} else if (strcmp(func, "migrate-image") == 0) {
			deactivate_migrate_image();
		} else if (strcmp(func, "sched-image") == 0) {
...
static void diag_cb_sys_enter(void *data, struct pt_regs *regs, long id)
{
	...
	if (id >= DIAG_BASE_SYSCALL) {
		...
		} else if (id >= DIAG_BASE_SYSCALL_TCP_STATES
		   && id < DIAG_BASE_SYSCALL_TCP_STATES + DIAG_SYSCALL_INTERVAL) {
			ret = tcp_states_syscall(regs, id);
		} else if (id >= DIAG_BASE_SYSCALL_UDP
		   && id < DIAG_BASE_SYSCALL_UDP + DIAG_SYSCALL_INTERVAL) {
			ret = udp_syscall(regs, id);
		} else if (id >= DIAG_BASE_SYSCALL_RESOURCE_IMAGE
		   && id < DIAG_BASE_SYSCALL_RESOURCE_IMAGE + DIAG_SYSCALL_INTERVAL) {
			ret = resource_image_syscall(regs, id);
		} else if (id >= DIAG_BASE_SYSCALL_MIGRATE_IMAGE
		   && id < DIAG_BASE_SYSCALL_MIGRATE_IMAGE + DIAG_SYSCALL_INTERVAL) {
			ret = migrate_image_syscall(regs, id);
		} else if (id >= DIAG_BASE_SYSCALL_SCHED_IMAGE
		   && id < DIAG_BASE_SYSCALL_SCHED_IMAGE + DIAG_SYSCALL_INTERVAL) {
			ret = sched_image_syscall(regs, id);
		} else if (id >= DIAG_BASE_SYSCALL_TEST_MAP
		   && id < DIAG_BASE_SYSCALL_TEST_MAP + DIAG_SYSCALL_INTERVAL) {
			ret = test_map_syscall(regs, id);
		} else if (id >= DIAG_BASE_SYSCALL_KEYTIME_IMAGE
		   && id < DIAG_BASE_SYSCALL_KEYTIME_IMAGE + DIAG_SYSCALL_INTERVAL) {
			ret = keytime_image_syscall(regs, id);
		}
...
static int __init diagnosis_init(void)
{
	...
	for (i = 0; i < num_possible_cpus(); i++) {
		diag_percpu_context[i] = (struct diag_percpu_context *)(diag_context_mem + i * sizeof(struct diag_percpu_context));
	}
