...
int diag_kernel_init(void)
{
	...
	ret = diag_resource_image_init();
	if (ret)
		goto out_resource_image; 
	ret = diag_migrate_image_init();
	if (ret)
		goto out_migrate_image;
	ret = diag_sched_image_init();
	if (ret)
		goto out_sched_image; 

	ret = diag_keytime_image_init();
	if (ret)
		goto out_keytime_image; 
	ret = diag_test_map_init();
	if (ret)
		goto out_test_map; 

	on_each_cpu(start_timer, NULL, 1);//在每个cpu上开启定时器
...
out_irq_delay:
	diag_irq_stats_exit();
out_resource_image:
	diag_resource_image_exit();
out_migrate_image:
	diag_migrate_image_exit();
out_sched_image:
	diag_sched_image_exit();
out_keytime_image:
	diag_keytime_image_exit();
out_test_map:
	diag_test_map_exit();
out:
	return ret;
